...
from src.config import CAPACITY, BLOCK_SIZE

# A simple dataset class that loads data from a file or accepts a list directly.
...
# Interface where datasets is managed during training (include updating block properties and sampling training data).
class BlkPosInterface:
    ...
    # Sampling training data for judge model training
    # Training data consists of:
    # 1. Continuous blocks randomly sampled from the text (Data distribution of retrieval competition)
    # 2. All relevant blocks + random irrelevant blocks   (Data distribution of rehearsal)
    def build_random_buffer(self, num_samples):
        n0, n1 = [int(s) for s in num_samples.split(',')][:2]
        ret = []
        max_blk_num = CAPACITY // (BLOCK_SIZE + 1)

        logging.info('Building buffers for judging...')
    ...
    # Sampling training data for reasoner model training
    # Training data consists of:
    # 1. All relevant block + remaining top-scored block by judge model
    # 2. All relevant block + random irrelevant blocks
...
def find_latest_checkpoint(checkpoints_dir, epoch=False):
    ...
    if checkpoints_dir.exists():
        for path in checkpoints_dir.iterdir():
            ...
            m = re.match(r'_ckpt_epoch_(\d+).+', path.name)
            if m is not None:
                ep = int(m.group(1))
                if ep > latest[0]:
                    latest = (ep, path)

    return latest[0] if epoch else latest[1]