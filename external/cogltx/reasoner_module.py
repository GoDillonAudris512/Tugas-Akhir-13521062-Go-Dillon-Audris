...
# - Directly use ClassificationReasoner class from models
...
from external.cogltx.models import *
from external.cogltx.buffer import buffer_collate
from src.config import CAPACITY

# Reasoner module to manage the reasoner model during training and inference
class ReasonerModule(pl.LightningModule):
    ...
    def __init__(self, config):
        ...
        reasoner_config = dict([(k[16:], v) for k,v in config.__dict__.items() if k.startswith('reasoner_config_')])
        self.reasoner = ClassificationReasoner.from_pretrained(config.model_name, **reasoner_config)
    ...
    def on_epoch_start(self):
        ...
        self._file = open(os.path.join(self.config.tmp_dir, 'changes_{}.txt'.format(device_name)), 'w')
    ...
    def configure_optimizers(self):
        optimizer = torch.optim.AdamW(
            ...
            lr=self.config.lr_reasoner,
            weight_decay=self.config.weight_decay_reasoner
            )
    ...
    # Intervention infers relevance labels for each blocks on classification task
    # Procedure done by testing whether the block is indispensable to the classification decision
    ...
    def training_step(self, bufs, batch_idx):
        ...
        # Extract the labels for reasoner
    ...
    def add_specific_args(parser):
        parser.add_argument('--lr_reasoner', type=float, default=3e-6, help='learning rate of reasoner')
        parser.add_argument('--weight_decay_reasoner', type=float, default=0.001, help='weight decay of reasoner')
        parser.add_argument('--batch_size_reason_per_gpu', type=int, default=2, help='gradient batch_size')
        parser.add_argument('--levelup_threshold', type=float, default=0.2, help='upper threshold')
        parser.add_argument('--leveldown_threshold', type=float, default=-0.05, help='lower threshold')
